#!/usr/bin/env python3
"""
setup_le_b2_lat12p8.py

Generate YAML configs for DS1 B2 label-efficiency benchmark with:
  - lat=12, proj=8
  - NO n_labeled_per_class (all budget labels go to SupCon loss)

Reuses annotation CSVs from le_b2_supcon (same folds, same budget subsets).
DS1 only (vinc ctrl+ycomp).

Jobs: 5 folds × (11 budgets × 5 repeats + 1 all) = 280

Usage:
  python scripts/setup_le_b2_lat12p8.py [--dry-run]
"""
...
RUN_TAG      = "le_b2_lat12p8"
SRC_ANN_TAG  = "le_b2_supcon"        # reuse these annotation CSVs
DS           = "ds1"
N_FOLDS      = 5
N_REPEATS    = 5
...
ANN_DIR      = DATA_ROOT / "labelling" / SRC_ANN_TAG  # read-only reuse

PATCH_DIRS = [
    dict(
        path      ="ae_results/patches/cio/vinc/control/tiff_patches32_mr10",
        frame_dir ="ae_results/source_frames/cio_mode_prt/vinc/control",
        condition =0, condition_name="control",
    ),
    dict(
        path      ="ae_results/patches/cio/vinc/ycomp/tiff_patches32_mr10",
    ...
    for fold in range(N_FOLDS):
        for budget in BUDGETS:
            for rep in range(N_REPEATS):
                ann_name = f"le_b2_{DS}_fv{fold}_nb{budget}_r{rep}"
                ann_path = ANN_DIR / f"{ann_name}.csv"
                if not ann_path.exists():
                    missing.append(str(ann_path))
                    continue
                name = f"le_b2_lat12p8_{DS}_fv{fold}_nb{budget}_r{rep}"
                _write(name, ann_name, fold, budget, rep, dry, job_list)

        # "all" budget — 1 repeat
        ann_name = f"le_b2_{DS}_fv{fold}_nball_r0"
        ann_path = ANN_DIR / f"{ann_name}.csv"
        if ann_path.exists():
            name = f"le_b2_lat12p8_{DS}_fv{fold}_nball_r0"
