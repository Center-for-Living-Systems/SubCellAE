#!/usr/bin/env python3
"""
setup_le_b12_ds1_lat64p32.py

Generate YAML configs for DS1 B12 label-efficiency benchmark with:
  - lat=64, proj=32
  - No n_labeled_per_class

Reuses annotation CSVs from le_b12_supcon (same folds, same budget subsets).
DS1 only (vinc ctrl+ycomp).
Budgets: 10,20,25,50,75,100,150,200,300,400,500,750,1000,1250,1500 + all
Jobs: 5 folds × (15 budgets × 5 repeats + 1 all) = 380

Usage:
  python scripts/setup_le_b12_ds1_lat64p32.py [--dry-run]
"""
...
RUN_TAG     = "le_b12_ds1_lat64p32"
SRC_ANN_TAG = "le_b12_supcon"
DS          = "ds1"
N_FOLDS     = 5
N_REPEATS   = 5
BUDGETS     = [10, 20, 25, 50, 75, 100, 150, 200, 300, 400, 500, 750, 1000, 1250, 1500]
        ...
        for budget in BUDGETS:
            for rep in range(N_REPEATS):
                ...
                if not ann_path.exists():
                    ...
                    continue
                name = f"le_b12_{DS}_lat64p32_fv{fold}_nb{budget}_r{rep}"
                _write(name, ann_name, fold, budget, rep, dry, job_list)

        ann_name = f"le_b12_{DS}_fv{fold}_nball_r0"
        ann_path = ANN_DIR / f"{ann_name}.csv"
        if ann_path.exists():
            name = f"le_b12_{DS}_lat64p32_fv{fold}_nball_r0"
