#!/usr/bin/env python3
"""
run_le_combined_s3v1.py
=======================
Evaluate combined image-count × label-count efficiency — s3v1 split.
  Train: frames 0+1+2  ·  Test: frame 3

Outputs
-------
  results/le_combined_s3v1_results.csv
  results/le_combined_s3v1_heatmap.png
  results/le_combined_s3v1_curves.png
"""
...
COMB_DIR  = DATA / "ae_results/contrastive_run/le_combined_s3v1"
PATCH_DIR = DATA / "ae_results/patches/cio/vinc/control/tiff_patches32_mr10"
ANN_DIR   = DATA / "labelling/le_combined_s3v1"
...
def _parse_run_name(name: str) -> dict | None:
    m = re.match(r"le_comb_s3_n(\d+)_npi(\d+)_r(\d+)$", name)
    if not m:
        return None
...
def run_one(run_dir: Path, full_ann: pd.DataFrame) -> dict | None:
    ...
    ann_csv = ANN_DIR / f"le_comb_s3_npi{npi}_r{series}.csv"
...
def _plot_curves(summary: pd.DataFrame, out: Path):
    ...
    ax.set_title("s3v1 Combined Efficiency — BAcc vs N_images\n"
                 "SupCon AE + LGBM · train frames 0+1+2 · test frame 3 · 3 series",
...
def main():
    ...
    run_dirs = sorted(COMB_DIR.glob("le_comb_s3_n*_npi*_r*"))
    if not run_dirs:
    ...
    df.to_csv(out_dir / "le_combined_s3v1_results.csv", index=False)
    print(f"Saved: {out_dir / 'le_combined_s3v1_results.csv'}")
    ...
    _plot_heatmap(summary, "s3v1 BAcc (%) · rows=npi · cols=N_images · test frame 3",
                  out_dir / "le_combined_s3v1_heatmap.png")
    _plot_curves(summary, out_dir / "le_combined_s3v1_curves.png")
