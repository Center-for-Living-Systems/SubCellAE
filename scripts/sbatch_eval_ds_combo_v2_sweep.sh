#!/usr/bin/env bash
#SBATCH --job-name=ds_combo_v2_eval
#SBATCH --partition=general
#SBATCH --gres=gpu:a40:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=48G
#SBATCH --time=08:00:00
#SBATCH --output=logs/slurm/ds_combo_v2_eval_%j.out

# Post-training eval + analysis for all 15 v2 dataset-combo models.
#   Stage 1: run_cross_dataset_eval.py sweep  → violin plots + metrics CSV
#   Stage 2: run_ds_combo_analysis.py per model → UMAP + cluster panels
...
RUNS="$ROOT/ae_results/contrastive_run/ds_combo_enlcrop_sc2_lc010_bal"
