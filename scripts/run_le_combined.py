...
run_le_combined.py
==================
Evaluate the combined image-count × label-count efficiency experiment.

For each le_comb_n{N:03d}_npi{npi}_r{series} directory:
  1. Load latents.csv  (encoded training-frame patches — always includes frame 0)
  2. Train LGBM on the npi cumulative labels from frame 0
  3. Encode test frames (1, 2, 3) with the saved model
  4. Compute balanced accuracy + per-class recall/precision

Outputs
-------
  results/le_combined_results.csv
  results/le_combined_heatmap.png    — BAcc as 2-D heat map (N_images × npi)
  results/le_combined_curves.png     — BAcc curves per npi level vs N_images
"""
...
def _parse_run_name(name: str) -> dict | None:
    m = re.match(r"le_comb_n(\d+)_npi(\d+)_r(\d+)$", name)
    if not m:
        return None
    return {"n_images": int(m.group(1)),
            "npi":      int(m.group(2)),
...
def _encode_frames(model: ContrastiveAE, frame_set: set[int],
    ...
    for i in range(0, len(patch_files), batch_size):
        ...
        imgs  = [tifffile.imread(str(p)).astype(np.float32) / INPUT_DIVISOR
                 for p in batch]
...
def run_one(run_dir: Path, full_ann: pd.DataFrame) -> dict | None:
    ...
    ann_csv  = ANN_DIR / f"le_comb_npi{npi}_r{series}.csv"
    ...
    # ── train LGBM on frame-0 labels ────────────────────────────────────────
    latents = pd.read_csv(lat_csv)
    latents["frame"] = latents["filename"].apply(_extract_frame)

    ann_train = pd.read_csv(ann_csv)
    train_lat = latents[latents["frame"] == TRAIN_FRAME]
    train_labeled = train_lat.merge(ann_train[["filename", "label"]],
                                    on="filename", how="inner")
    if len(train_labeled) == 0:
        print(f"  [skip] no training labels in latents: {run_dir.name}")
        return None
    ...
    # ── encode test frames with trained model ────────────────────────────────
    ...
    test_labeled = test_enc.merge(full_ann[["filename", "label"]],
                                  on="filename", how="inner")
    ...
    print(f"  {run_dir.name}  BAcc={bacc:.3f}  "
...
def _plot_heatmap(summary: pd.DataFrame, metric: str, title: str, out: Path):
    """2-D heat map: rows = npi, cols = N_images, value = mean metric."""
    piv = summary.pivot(index="npi", columns="n_images",
                        values=f"{metric}_mean") * 100
    piv = piv.reindex(index=NPI_LEVELS, columns=N_VALUES)

    fig, ax = plt.subplots(figsize=(13, 4), facecolor="white")
    im = ax.imshow(piv.values, aspect="auto", cmap="YlGn",
    ...
    ax.set_yticks(range(len(NPI_LEVELS)))
    ax.set_yticklabels([str(n) for n in NPI_LEVELS], fontsize=9)
    ax.set_xlabel("N images in AE training", fontsize=10)
    ax.set_ylabel("Labels per image (npi, cumulative)", fontsize=10)
    ax.set_title(title, fontsize=10, fontweight="bold")

    for ri, npi in enumerate(NPI_LEVELS):
...
def _plot_curves(summary: pd.DataFrame, out: Path):
    """BAcc curves: one line per npi level, x = N_images (log scale)."""
    colors = ["#d62728", "#ff7f0e", "#2ca02c", "#1f77b4", "#9467bd"]
    fig, ax = plt.subplots(figsize=(10, 5.5), facecolor="white")

    x = np.log2(N_VALUES)
    for npi, color in zip(NPI_LEVELS, colors):
        s = summary[summary["npi"] == npi].sort_values("n_images")
        means = s["balanced_acc_mean"].values * 100
        stds  = s["balanced_acc_std"].values  * 100
    ...
    ax.set_title(
        "Combined Efficiency — BAcc vs N_images, coloured by npi (cumulative labels)\n"
        "SupCon AE + LGBM · frame 0 trains · frames 1,2,3 test · 3 series",
...
def main():
    ...
    run_dirs = sorted(COMB_DIR.glob("le_comb_n*_npi*_r*"))
    if not run_dirs:
        print(f"No run directories found in {COMB_DIR}")
        return
    ...
    df.to_csv(out_dir / "le_combined_results.csv", index=False)
    print(f"Saved: {out_dir / 'le_combined_results.csv'}")
    ...
    _plot_heatmap(
        ...
        "Combined Efficiency — BAcc (%) · rows=npi (cumulative) · cols=N_images",
        out_dir / "le_combined_heatmap.png",
    )
    _plot_curves(summary, out_dir / "le_combined_curves.png")
